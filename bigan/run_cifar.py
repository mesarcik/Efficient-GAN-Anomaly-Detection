...
import importlib
import sys
import bigan.cifar_utilities as network
import data.cifar as data
...
def create_logdir(method, weight, label, rd):
    ...
    return "bigan/train_logs/cifar/{}/{}/{}/{}".format(weight, method, label, rd)

def train_and_test(nb_epochs, weight, method, degree, random_seed, label):
    """ Runs the Bigan on the CIFAR dataset
    ...
    """
    logger = logging.getLogger("BiGAN.train.cifar.{}.{}".format(method,label))
    ...
    with sv.managed_session() as sess:
        ...
        fill = np.ones([batch_size - size, 32, 32, 3])
        ...
        prc_auc = do_prc(scores, testy,
               file_name=r'bigan/cifar/{}/{}/{}'.format(method, weight,
                                                     label),
               directory=r'results/bigan/cifar/{}/{}/'.format(method,
                                                           weight))
